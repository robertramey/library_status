...
namespace
{
    ...
    struct col_node {
        ...
        col_node() :
            has_log_file(false)
        {}
        std::pair<int, int> calculate_spans();
    };
    ...
    std::pair<int, int> col_node::calculate_spans(){
        ...
        if(! m_subcolumns.empty()){
            BOOST_FOREACH(
                ...
                spans = s.second.calculate_spans();
        ...
        BOOST_FOREACH(
            ...
            std::make_pair(
             ...
             if(d.path().filename() == "test_log.xml"){
                 node.has_log_file = true;
                 node.m_subcolumns.insert(
                    std::make_pair("", col_node())
                );
             }
        }
    }
    ...
    void emit_column_headers(
        ...
        /*
        if(node.has_log_file && ! node.m_subcolumns.empty()){
    ...
    void do_table(const fs::path & lib_test_dir, const string & lib_name)
    {
        ...
        // emit
        root_node.calculate_spans();
        int row_count = root_node.rows - 1;
        report << "<tr>\n";
